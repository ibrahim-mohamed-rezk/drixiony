...
function Modals() {
  ...
  const [loginFormData, setLoginFormData] = useState({ phone: null });
  const [createAccountStatus, setCreateAccountStatus] = useState("form");
  ...
  const handlCreateAccount = async (e) => {
    ...
    try {
      ...
      setOtp((preve) => ({ ...preve, phone: createAccountFormData.phone }));

      // Close the sign up modal
      setCreateAccountStatus("otp");

      setCreateAccountFormData({ name: null, phone: null });
    } catch (error) {
  ...
  const handleLogin = async (e) => {
    e.preventDefault();
    try {
      await postData("login", loginFormData, {
        "Content-Type": "multipart/form-data",
      });
      toast.success("تم ارسال الكود الي الواتساب");
      setOtp((preve) => ({ ...preve, phone: loginFormData.phone }));
      // Close the sign up modal
      setLogintStatus("otp");
      setLoginFormData({ phone: null });
    } catch (error) {
      if (axios.isAxiosError(error)) {
        toast.error(error.response?.data?.message || "An error occurred");
      } else {
